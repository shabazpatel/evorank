...
relevance, conversion, and revenue as competing objectives, three seeds each
converge within 50 iterations (about ten dollars) on interpretable pipelines
that beat an Optuna-tuned LambdaMART on 60k held-out queries, an advantage
that persists at full data scale and would have placed in the top 6 percent
of the original competition via one-shot late submission. A first campaign of the same
loop, evolving only training objectives, supplies the study's central design
rule: it appeared to work on its selection fold while a transfer audit
showed the gains were almost entirely fitness noise (the run-to-run
randomness of the loop's internal scoring), and neither seeded domain
knowledge nor richer diagnostic feedback changed what transferred. The deciding quantity is
...
catalog of failure modes with their guardrails, so teams can apply the same
procedure to their own ranking stacks.
\end{abstract}
...
(1) a quantified boundary condition for when such loops work: evolved
training objectives whose gains on the selection fold (the small dataset the
loop scores candidates on) do not survive a held-out transfer audit, for a
reason teams can check in their own settings, real effect sizes smaller than
the noise floor of the loop's own scoring; (2) guidance ablations showing
that seeded domain knowledge and evidence-rich diagnostic feedback change the
speed and style of search but not what generalizes; (3) the constructive counterpart
...
better on every objective at once, in all three seeds, which reconstruct and extend the
...
experimentation to production ranking behind closed doors. We contribute the missing measurement layer: an
...
accuracy. Our deltas are the ranking domain (query-grouped data, within-query rank
features, per-query ensembling), transfer-audited selection as the
acceptance criterion, and a quantified catalog of agent-specific
target-encoding leakage modes.
...
\begin{figure}
  ...
  small fold the loop scores candidates on and a large held-out fold it
  never sees. Lane B: the evolution loop, one LLM call per iteration: the
  archive supplies a parent and rivals, the LLM edits the marked blocks, the
  candidate is trained and scored under guardrails, and plain-language
  feedback (Figure~2) travels back with the program; broken candidates
  return explanations instead of scores. The dashed box is optional expert
  knowledge, ablated in campaign 1. Lane C: the honesty harness: a pre-spend
  gate, baselines on equal terms, and the held-out audit that decides what
...
search may change. Each iteration samples a parent (plus context programs) from a
...
back into the database. In classical terms, the LLM replaces hand-designed mutation operators and
the prompt context plays the role of the mating pool.
...
island adapts its exploration intensity accordingly. The database keeps a Pareto archive, the set of best available trade-offs
over the three objectives (ndcg, booking ndcg, pooled dollar-weighted
revenue), with a scalar key used only for tie-breaking. On stagnation the controller can prompt the LLM for a
high-level strategy change rather than a local edit. All state is checkpointed, so runs resume exactly and every
...
labels. NDCG@k scores how well a model orders each list: relevant items near
the top earn gain discounted by log-position, normalized by the best
achievable ordering, so 1.0 is perfect. We average it per query (for revenue
...
evaluates candidates on a small fitness fold of 8k training queries: a
candidate trains a gradient-boosted ranker under a fixed budget and is
scored on the fold's held-back validation queries, and that score is its
fitness; the small fold keeps each evaluation between 5 and 40 seconds. Some
...
own feedback; (b) equal-budget random-search controls; (c) symmetric
hyperparameter treatment (post-hoc Optuna tuning of discovered artifacts);
(d) a selection-generalization audit that rescores every checkpoint's
selection on 60k held-out queries; (e) three-objective hypervolume; and (f)
mechanism ablations of discovered artifacts. Baselines are LambdaMART (the standard
...
The evolvable artifact is the LambdaMART gradient function, computed per
query group (the set of hotels returned for one search), with access to each
impression's raw behavioral signals; training configuration, features, and
...
picking the best score means picking luck. Enlarging the fitness fold's validation side from 1.6k to 6k queries halves
that noise and triples the mean gain surviving transfer; it is the only
...
run; unseeded runs need 7 to 23. Seeded runs also end with a better
three-objective frontier (higher hypervolume) in every seed. What transfers
to held-out data is unchanged. A diagnostic feedback channel (noise-gated deltas, per-segment breakdowns,
probes into the candidate's training gradients) changes nothing measurable
across three seeds. One probe byproduct: the best discovered revenue term moves only 0.09
percent of the total training gradient yet removing it costs 0.009 revenue,
so even tiny but systematic pressure compounds across boosting rounds.
...
The LLM now edits two blocks: feature-construction code over semantically named columns, where any
statistic touching labels must come from provided helper functions with the
leakage guards built in (counts, out-of-fold rates, quantiles), and a
pipeline specification chosen from a whitelist (one to three members from XGBoost,
LightGBM, and sklearn families; rank, pointwise, binary, or campaign-1
losses; z-score or rank-mean combining). Feedback is stage-attributed so the
loop can tell which axis earned each change; Figure~\ref{fig:feedback}
shows it verbatim, exactly as it appears in the next prompt.
...
Before any LLM spend, the headroom gate: a hand-built candidate from the ICDM 2013 literature must beat the noise
floor by a wide margin (we required 2.5 times, so real headroom cannot be
mistaken for luck), and within-query rank features plus count encodings did
($+0.013$ NDCG@10). Building that candidate surfaced three successively subtler leakage bugs,
...
was caught by the audit before any search depended on it.
...
What the search found (Figure~\ref{fig:discovered}): three seeds, at 50
...
models blended by per-query z-scores. One provenance
distinction matters: the seeded priors included the gate's finding that rank
and count features carry headroom, so their presence is partly prompted; the
specific constructions, the model, loss, ensemble, and weight selections,
and the unprompted inventions (per-query price z-normalization, quantile
imputation of a heavy-missing location score) are the search's own, and the
transfer-audited gains are independent of provenance.

\begin{figure}
\footnotesize
\begin{verbatim}
# discovered by seed s1 (excerpt); comments are the model's own
for c in ["price_usd", "prop_starrating", "prop_location_score2",
...
\end{verbatim}
\caption{The discovered artifact is readable code: feature block excerpt and
pipeline specification from seed s1's final program (full programs in the
repository).}
\label{fig:discovered}
\end{figure}

Does it transfer? Table~\ref{tab:transfer}: roughly 70 percent of
selection-fold gains survive on 60k held-out queries, and all three seeds
beat the tuned baseline. A paired query bootstrap (10,000 resamples) makes
this formal: the best seed wins by +0.0057 NDCG@10 (95 percent CI [0.0041,
...
Training on the full dataset required its own rigor step and yields a
methodology lesson (Table~\ref{tab:fullscale}): against a baseline whose
hyperparameters were tuned on the small fold the pipelines win easily, but
re-tuning the baseline at full scale reverses that naive verdict, and only a
comparison with the same tuned hyperparameters on both sides isolates the
...
\begin{figure}
  ...
  \caption{The study in one picture: per-seed NDCG@10 gains over the seed
  program on the selection fold (open) and on 60k held-out queries (filled).
  Campaign 1's selection-fold gains collapse into the held-out noise band;
  campaign 2's survive.}
  \label{fig:contrast}
\end{figure}
...
achievable effect sizes to fitness-fold noise. We suggest the headroom gate as cheap preregistration for discovery
loops, and transfer-audited selection as the reporting standard. The result is not specific to the search algorithm: a
recalibrated AdaEvolve and an EvoX meta-evolution variant~\cite{evox}, each
at double budget, land within the original seed spread on the held-out
fold. Together these results locate the leverage for future work in the
measurement layer rather than the search controller: guidance and algorithm
sophistication changed the speed and style of search, while evaluation
fidelity, leakage-safe statistics, and rejection messages the model learns
from moved outcomes. The clearest next step is multi-fidelity evaluation:
screen candidates on the cheap fold and promote survivors to full-scale
evaluation before selection, exposing scale-dependent features (per-entity
rates, position effects) that a small fitness fold cannot see and that
account for much of our remaining gap to the 2013 winners. The economics compound this: a campaign costs about ten dollars and two
unattended hours, so the relevant unit is gain per human hour; the
solutions our loop reconstructs took a seven-person team a season.
...
held-out audit, not the loop's own scores, decide what counts. The loop is
neither magic nor broken: its discoveries are real precisely when the search
space offers effect sizes the fitness signal can resolve, and both
quantities are measurable before any budget is spent. Code, configs, seeds,
