...
def lambdamart_objective(predt, rel, booking, rev, group_slices, sigma=1.0):
    ...
    for s, e in group_slices:
        ...
        # Revenue tie-breaker: bounded, log-scaled, only nudges ordering
        # among items sharing the same relevance grade (mostly booked rows).
        log_rev = np.log1p(rv)
        max_log_rev = log_rev.max()
        rev_norm = log_rev / max_log_rev if max_log_rev > 0 else log_rev
        combo = lab.astype(np.float64) + 0.25 * rev_norm  # rel dominates, revenue breaks ties

        M = (combo[:, None] > combo[None, :])           # i strictly "more valuable" than j
        gain = (2.0 ** lab - 1.0) + 0.5 * rev_norm       # relevance gain + small revenue bonus
        ranks = np.empty(m, dtype=int); ranks[np.argsort(-sc, kind="stable")] = np.arange(m)
        disc = 1.0 / np.log2(ranks + 2.0)
        ideal_order = np.argsort(-combo, kind="stable")
        idcg = float(np.sum(gain[ideal_order] * (1.0 / np.log2(np.arange(2, m + 2))))) or 1.0
