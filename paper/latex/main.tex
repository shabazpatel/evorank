...
6.9M-impression dataset. The discovered pipelines reconstruct and extend the
...
audit showed the gains were almost entirely fitness noise, and neither
...
a scalar fitness. We bring loss search to supervised multi-objective GBDT ranking via LLM
mutations over full gradient code, and report a negative this literature
lacks: the family's headroom sits below any loop-affordable noise floor, so
discovered losses do not transfer.
...
\begin{figure}
  ...
  fast fitness fold and a large held-out fold. Lane B: the evolution loop,
  one LLM call per iteration, cycling from the Pareto program database
  through prompt building, LLM mutation of the candidate's EVOLVE blocks
  (campaign 1: a gradient objective; campaign 2: feature code plus a model,
  loss, and ensemble specification), guardrailed evaluation, and
  three-objective scoring; degenerate candidates return explanatory
  rejection messages. The seeded knowledge base (dashed) is the campaign 1
  ablation treatment. Lane C: the honesty harness, the headroom gate before
  any spend, per-regime-tuned reference baselines, and the transfer audit
  that rescores every selected program on the held-out fold.}
  \label{fig:system}
\end{figure}
...
optimal configurations change with training-set size.
...
three seeds on every endpoint. One probe byproduct: the best discovered revenue term moves only 0.09
...
evaluations. At full scale the family is a plateau: everything, including
default rank:ndcg, lands within 0.001 NDCG@10.
...
Figure~\ref{fig:feedback} shows the feedback artifact verbatim; this
string, attached to each candidate in the prompt, is what the search learns
from.
