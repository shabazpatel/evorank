...
def lambdamart_objective(predt, rel, booking, rev, group_slices, sigma=1.0):
    ...
    alpha = 0.75  # weight on booking/revenue lambda term relative to rel term
    for s, e in group_slices:
        ...
        rev_rank = np.empty(m, dtype=np.float64)
        rev_rank[order] = np.arange(m, dtype=np.float64)
        rev_norm = rev_rank / (m - 1) if m > 1 else np.zeros(m)
        rev_norm = np.where(rv > 0, rev_norm, 0.0)
        click_only = (lab == 1.0) & (bk == 0.0)
        gain_bk = 2.0 * bk + rev_norm + 0.1 * click_only
