...
def lambdamart_objective(predt, rel, booking, rev, group_slices, sigma=1.0):
    ...
    hand-set choice.
    """
    # Combined-gain LambdaMART: fold rel, booking, and revenue into one
    # per-query normalized gain (each scaled to its own [0,1] max so no
    # signal dominates by raw magnitude), then run standard |deltaNDCG|
    # pairwise lambda on that composite gain. This jointly pushes ordering
    # toward relevance, conversion, and revenue simultaneously.
    ...
    for s, e in group_slices:
        ...
        # Log-compress revenue, scale by a robust (90th percentile) of the
        # group's log-revenue so a single extreme booking doesn't crush the
        # gain spacing for everyone else, then apply a smooth (tanh) soft
        # saturation instead of a hard clip so gradients stay informative
        # even for outsized revenue values instead of vanishing abruptly.
        ...
        rev_norm = 1.5 * np.tanh(rev_log / rev_scale)
