...
the loop will pay off, and we release the tool, the audit stack, and a catalog of
failure modes with the guardrails that catch them, so teams can apply the
same procedure to their own ranking stacks.
\end{abstract}
...
What the search found: three seeds, 50 iterations and about ten dollars
each, converged on the same interpretable core: within-query percentile
ranks of price, star rating, review and location scores, count encodings of
property and destination identifiers, a lean 20 to 30 column feature set
whose comments cite the measured bloat evidence, and a diverse three-member
ensemble under per-query z-score weighting. One provenance
...
at double budget, land within the original seed spread on the held-out fold,
consistent with headroom and evaluation fidelity, not search sophistication,
governing outcomes. Together these results locate the leverage for future work in the
