...
def lambdamart_objective(predt, rel, booking, rev, group_slices, sigma=1.0):
    ...
    for s, e in group_slices:
        ...
        # Clip extreme per-query revenue outliers before log so a single very
        # expensive booking doesn't dominate the pairwise ordering/weighting.
        if rv.max() > 0:
            cap = np.percentile(rv, 95)
