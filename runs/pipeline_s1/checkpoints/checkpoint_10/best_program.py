...
def build_features(df, stats):
    """Feature set built from the fold's measured priors: within-query rank
    transforms (top single lever, +0.011 ndcg) combined with count encodings
    of high-cardinality ids (+0.013 combined, best hand-built variant), plus
    a small set of composite price/quality anchors and the stacked score.
    Kept intentionally lean (well under 150 columns) to avoid the kitchen
    -sink regression observed on this fold. Missing prop_location_score2 is
    imputed with a low train-fold quantile rather than zero."""
    import numpy as np

    out = {}

    # --- raw semantic features (kept as-is, cheap and informative) ---
    raw_cols = [
    ...
    # impute location_score2 with a low train-fold quantile (beats zero-fill)
    ...
    # --- within-query rank transforms (top single lever on this fold) ---
    ...
    # --- count encodings of high-cardinality ids (best hand-built variant) ---
    for c in ["prop_id", "srch_destination_id"]:
        if c in df.columns:
            out[f"{c}_count"] = stats.count(df, c)

    # --- composite price / quality anchors ---
    ...
    # --- stacked linear score on visitor/query columns ---
    out["stacked_score"] = stats.stacked_score(df)

    # --- aggregate competitor rate signal (not per-prop, avoids sparsity) ---
    comp_rate_cols = [c for c in df.columns if c.startswith("comp") and
                       c.endswith("_rate") and not c.endswith("_rate_percent_diff")]
    if comp_rate_cols:
