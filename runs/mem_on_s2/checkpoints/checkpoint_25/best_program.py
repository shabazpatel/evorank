...
def lambdamart_objective(predt, rel, booking, rev, group_slices, sigma=1.0):
    ...
    for s, e in group_slices:
        ...
        # Pure graded-relevance gain drives ranking direction, IDCG, and disc
        # exactly as standard NDCG (keeps ndcg / book_ndcg well-behaved).
        gain = (2.0 ** lab - 1.0)
        ...
        # Revenue amplification on the pair weight, scaled by the revenue of
        # the *more relevant* row (rev==0 unless booked, so click-vs-nonclick
        # pairs driving ndcg/book_ndcg are untouched). log1p-ratio clipped at
        # 1.0 gives a bounded, well-behaved multiplier without a tanh's early
        # saturation, matching the configuration empirically shown to lift
        # revenue while preserving ndcg/book_ndcg.
