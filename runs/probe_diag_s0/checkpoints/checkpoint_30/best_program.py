...
def lambdamart_objective(predt, rel, booking, rev, group_slices, sigma=1.0):
    ...
    for s, e in group_slices:
        ...
        # Clip extreme per-query revenue outliers before log (tighter 90th
        # pct cap than before) so a single very expensive booking doesn't
        # dominate the pairwise ordering/weighting, while a slightly higher
        # w_rev lets revenue carry a bit more signal in the composite gain.
        if rv.max() > 0:
            cap = np.percentile(rv, 90)
            rv_c = np.minimum(rv, cap)
        else:
            rv_c = rv
        gain = (2.0 ** r - 1.0) + w_book * b + w_rev * np.log1p(rv_c)
        S = sc[:, None] - sc[None, :]
        rho = 1.0 / (1.0 + np.exp(sigma * S))          # sigmoid(-sigma * S)
        M = (gain[:, None] > gain[None, :])            # i strictly more relevant than j
