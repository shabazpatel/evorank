...
def lambdamart_objective(predt, rel, booking, rev, group_slices, sigma=1.138):
    ...
    for s, e in group_slices:
        ...
        if m < 2:
            continue
        rev_term = np.log1p(v)
        gain = 0.881 * (2.0 ** r - 1.0) + 19.092 * b + 2.499 * rev_term
        S = sc[:, None] - sc[None, :]
        rho = 1.0 / (1.0 + np.exp(sigma * S))
        M = (gain[:, None] > gain[None, :])
        ranks = np.empty(m, dtype=int); ranks[np.argsort(-sc, kind="stable")] = np.arange(m)
        disc = 1.0 / np.log2(ranks + 2.0) ** 1.43
        ideal = np.sort(gain)[::-1]
        idcg = float(np.sum(ideal * (1.0 / np.log2(np.arange(2, m + 2)) ** 1.43))) or 1.0
