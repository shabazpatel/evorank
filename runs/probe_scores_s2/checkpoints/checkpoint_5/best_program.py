...
def lambdamart_objective(predt, rel, booking, rev, group_slices, sigma=1.0):
    ...
    hand-set choice.
    """
    """Rank-percentile revenue-augmented LambdaMART.

    Base gain is the standard relevance gain (2**rel - 1). Among booked rows
    (which usually all share the top relevance grade and are otherwise tied),
    we add a scale-invariant bonus equal to the within-query percentile rank
    of their revenue (0..1), scaled to match the magnitude of the top
    relevance gain (31 = 2**5-1). This creates real pairwise pressure to
    rank higher-revenue bookings above lower-revenue ones without being
    distorted by outlier revenue values (unlike raw/log revenue), since a
    percentile is bounded in [0,1] regardless of the underlying scale. The
    combined gain then drives both the pairwise mask and the |deltaZ|
    NDCG-style weighting, exactly as in the relevance-only baseline.
    """
    grad = np.zeros_like(predt, dtype=np.float64)
    hess = np.zeros_like(predt, dtype=np.float64)
    gamma = 0.5  # weight of revenue-percentile bonus relative to top rel gain
