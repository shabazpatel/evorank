...
RankEvolve~\cite{rankevolve} applies evolutionary search to unsupervised lexical retrieval with
...
remains the GBDT ranking standard; the ICDM 2013 challenge
solutions~\cite{liu2013} document the feature
recipes and per-query z-score ensembles that won; a 2024 survey~\cite{survey2024} notes that
...
program search in the AlphaEvolve family~\cite{alphaevolve,adaevolve}. The unit of evolution is a source
...
learning rate, tree depth, subsampling, and regularization.
Optuna~\cite{optuna} searches
...
the discovered structure, which then wins on all four reported numbers. A
...
rejection messages the model learns from) carried more of the outcome. The
economics compound this: a full campaign costs about ten dollars and two
unattended hours, so the relevant unit is gain per human hour, and the
hand-built solutions whose structure our loop reconstructs took a
seven-person team a competition season.
