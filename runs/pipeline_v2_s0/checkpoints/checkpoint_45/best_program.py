...
def build_features(df, stats):
    """Lean recipe (documented top-2 lifts on this fold): within-query rank
    transforms of price/star/review/location-score columns + high-cardinality
    count encodings (prop_id, srch_destination_id) + a handful of raw/derived
    anchors (price, stars, review, location score with low-quantile impute,
    promotion/brand flags, ump, price_diff) + stacked_score. Kept small
    (~14 cols) since kitchen-sink feature sets underperform this focused set."""
    import numpy as np
    import pandas as pd
    out = {}
    ...
    # competitive-pricing signal: mean of comp*_rate (+1 = competitor
    # cheaper, -1 = competitor pricier, 0/NaN = no data), a causal proxy for
    # price competitiveness distinct from the raw price/rank features.
    # Validated to lift ndcg, book_ndcg, and revenue together on this fold
    # when stacked onto the lean rank+count recipe.
...
PIPELINE = {
    ...
    "models": [
        ...
        # Deeper/more trees than a prior variant gave a validated gain
        # across all three objectives on this fold.
        {"family": "sklearn", "objective": "ert",
         "params": {"n_estimators": 150, "max_depth": 12,
                    "min_samples_leaf": 3}},
    ],
    # per-query z-score weighted sum; favor the strongest single ranker
    # (xgb rank:ndcg) while keeping lgbm lambdarank and ert for diversity;
    # validated on this fold to beat the more even 0.45/0.4/0.15 split
    # across ndcg, book_ndcg, and revenue.
