...
def build_features(df, stats):
    """Lean feature set: within-query rank transforms (top single lever,
    +0.011 ndcg on this fold) plus count encodings of high-cardinality ids
    (+0.013 combined, best hand-built variant), a small set of composite
...
PIPELINE = {
    ...
    # Third member is a genuinely different family (sklearn ert, bagged
    # non-boosted high-variance trees) rather than another xgb loss variant,
    # matching the validated diversity lever on this fold. Weight kept
    # modest since it is the weakest single member; two strongest
    # LambdaMART-family members carry most of the weight.
