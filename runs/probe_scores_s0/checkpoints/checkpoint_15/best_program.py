...
def lambdamart_objective(predt, rel, booking, rev, group_slices, sigma=1.0):
    ...
    hand-set choice.
    """
    """Revenue-aware LambdaMART: ties in relevance broken by revenue, and the
    |deltaNDCG| swap-magnitude gain blends in per-query max-normalized revenue
    (bounded in [0,1]) instead of raw/log revenue, so queries with very
    different price scales contribute comparably. IDCG still normalizes on
    pure relevance gain to keep NDCG/book_NDCG alignment intact.
    """
    grad = np.zeros_like(predt, dtype=np.float64)
    hess = np.zeros_like(predt, dtype=np.float64)
    alpha = 0.6  # weight of normalized-revenue term in the swap-magnitude gain
    for s, e in group_slices:
        ...
        # base relevance pair mask, plus revenue tie-break when relevance equal
        M = (lab[:, None] > lab[None, :]) | (
            (lab[:, None] == lab[None, :]) & (r[:, None] > r[None, :])
        )
        gain = (2.0 ** lab - 1.0)
        rmax = r.max()
        r_norm = r / rmax if rmax > 0 else np.zeros_like(r)
        comb_gain = gain + alpha * r_norm
        ...
        dZ = np.abs((comb_gain[:, None] - comb_gain[None, :]) * (disc[:, None] - disc[None, :])) / idcg
