...
def lambdamart_objective(predt, rel, booking, rev, group_slices, sigma=1.0):
    ...
    hand-set choice.
    """
    """Rank-percentile + normalized-log revenue-augmented LambdaMART.

    Base gain is the standard relevance gain (2**rel - 1). Among booked rows
    we add a bounded revenue bonus equal to a 50/50 blend of:
      (a) the within-query percentile rank of revenue (order-only signal,
          robust to outliers, bounded in [0,1]), and
      (b) log1p(revenue) normalized by the max log-revenue in the query
          (magnitude-aware signal, also bounded in [0,1] per query).
    The blend keeps the bonus scale-invariant and bounded like the winning
    percentile-only approach, while additionally rewarding *how much* higher
    one booking's revenue is relative to others in the same query (not just
    its rank), which should sharpen the revenue-ordering pressure without
    reintroducing raw-scale instability. This combined gain drives both the
    pairwise mask and the |deltaZ| NDCG-style weighting, exactly as in the
    relevance-only baseline.
    """
    grad = np.zeros_like(predt, dtype=np.float64)
    hess = np.zeros_like(predt, dtype=np.float64)
    gamma = 0.5  # weight of revenue bonus relative to top rel gain (31)
    for s, e in group_slices:
        ...
        if idx.size >= 1:
            ...
            gain = gain + gamma * 31.0 * rev_grade
        S = sc[:, None] - sc[None, :]
