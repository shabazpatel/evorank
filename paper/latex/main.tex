...
spend) on interpretable pipelines that Pareto-dominate an Optuna-tuned
LambdaMART on all three objectives on 60k held-out queries, an advantage that
persists under config-equalized comparison when training on the full
6.9M-impression dataset. The discovered pipelines independently reconstruct
and extend the load-bearing structure of the original challenge winners'
hand-built solutions. What makes these claims trustworthy is the built-in
honesty harness, and it earns its place through a measured counterexample: an
earlier campaign of the same loop, evolving only training objectives,
appeared to work on its selection fold while a transfer audit showed the
gains were almost entirely fitness noise (per-edit effect sizes below the
noise floor), and neither seeded domain knowledge nor gradient-introspective
diagnostic feedback changed what transferred. The deciding quantity is
...
\textbf{LLM-guided program evolution.} FunSearch and AlphaEvolve pair an LLM
mutation operator with evolutionary search over programs and automated
evaluators, with open engines reproducing the pattern; industrial ranking
agents (REA, GEARS) apply agentic experimentation to production ranking
behind closed doors. Results in this genre are overwhelmingly reported on the
...
provably discovering in another, with the deciding quantity (headroom versus
fitness noise) measurable before any spend, in the first open, reproducible
instance of the pattern for tabular LTR.

\textbf{Automated loss discovery.} GLO, AM-LFS, AutoLoss-Zero, and
CSE-Autoloss search loss functions by genetic programming over primitives,
mostly for vision; LambdaLoss is the hand-derived metric-bound counterpart;
RankEvolve applies evolutionary search to unsupervised lexical retrieval with
a scalar fitness. We bring loss search to supervised multi-objective GBDT
ranking with LLM mutations over full gradient code, and report the outcome
honestly: on this dataset the objective family's headroom sits below any
loop-affordable noise floor, so discovered losses do not transfer, a negative
result this literature has no analog of.

\textbf{Reflective feedback for LLM optimization.} Reflexion, ProTeGi, OPRO,
and GEPA show that richer textual feedback (traces, textual gradients)
improves LLM-driven search. We test the assumption under controlled
conditions and measure its boundary: gradient-introspective feedback, a new
feedback type possible because the evolved artifact is a differentiable loss,
changed nothing about what transferred across three seeds, while a data-side
fix to the fitness signal tripled transfer. Feedback content proved a speed
and interpretability lever, not a truth lever.

\textbf{Agentic AutoML and LLM feature engineering.} CAAFE generates features
for tabular classification; AIDE, ML-Agent, MLZero, and related agents
iterate over ML pipelines, typically evaluated by single-objective validation
...
\textbf{LTR foundations and the benchmark.} LambdaMART remains the GBDT
ranking standard; the ICDM 2013 challenge solutions document the feature
recipes and per-query z-score ensembles that won; a 2024 survey notes that
public datasets with monetary outcome labels remain scarce, anchoring open
...
two hours for about ten dollars, and beats a per-regime-tuned LambdaMART on
all objectives at every scale tested.
...
\emph{GBDT and LambdaMART.} Gradient-boosted decision trees (XGBoost,
LightGBM) fit an additive sequence of small trees, each correcting the
...
learning rate, tree depth, subsampling, and regularization. Optuna searches
that space with a Bayesian strategy (TPE), proposing configurations, scoring
each on validation data, and concentrating trials where scores improve. Our
baselines get 40 trials, re-run in every data regime we report, because
optimal depth and round counts change with training-set size.

\emph{Ensembling by per-query z-scores.} Different model families make
partially independent errors. Scores from different models live on different
scales, so within each query we standardize every model's scores to zero mean
and unit variance, then take a weighted sum. This was the combiner of choice
among the original challenge winners and is what our discovered pipelines
select.

\emph{Out-of-fold target encoding.} Encoding a categorical column by the
average label of its value (for example, a hotel's historical booking rate)
is powerful but leaks labels into features if computed naively. The safe
construction splits the training data into folds by query and encodes each
row using statistics from the other folds only. Section 6 quantifies what
goes wrong otherwise.

\emph{The statistics.} A paired query bootstrap resamples queries with
replacement and recomputes a metric difference between two systems thousands
of times, yielding confidence intervals that respect the query as the unit of
noise. Pareto dominance says pipeline A beats B only if A is at least as good
on every objective and better on one; hypervolume measures how much objective
space a set of non-dominated solutions covers.
