...
def lambdamart_objective(predt, rel, booking, rev, group_slices, sigma=1.0):
    ...
    for s, e in group_slices:
        ...
        # Revenue amplification: scale pair magnitude by the revenue of the
        # *more relevant* row (rw==1 whenever that row isn't a booking, since
        # rev==0 there), so click-vs-nonclick pairs (which drive ndcg /
        # book_ndcg) are left untouched while high-value bookings get pushed
        # harder toward rank 1, lifting the dollar-weighted revenue metric.
