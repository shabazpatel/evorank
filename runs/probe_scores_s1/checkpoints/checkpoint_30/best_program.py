...
def lambdamart_objective(predt, rel, booking, rev, group_slices, sigma=1.0):
    ...
    for s, e in group_slices:
        ...
        # --- Term 2: single combined booking+revenue lambda. Every booked
        # row gets a baseline gain of 0.5 (pure conversion signal, lifts
        # book_ndcg even without revenue spread) plus up to 0.5 more from its
        # within-query percentile rank of revenue among booked rows. Fusing
        # booking and revenue into one term (instead of two separate ones)
        # avoids double-counting conversion signal and empirically improves
        # all three metrics over splitting them. ---
