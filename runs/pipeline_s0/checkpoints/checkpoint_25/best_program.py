...
def build_features(df, stats):
    """Rank+count recipe (best documented hand-built variant, +0.013 ndcg):
    within-query percentile ranks of price/star/review/location-score
    columns (listwise signal), count encodings of prop_id and
    srch_destination_id (largest single documented gain), a handful of
    causal composite price/star anchors (ump, price_diff, starrating_diff),
    quantile-imputed prop_location_score2, plus a small set of raw core
    columns. Kept to ~18 columns to avoid feature-bloat dilution."""
    import numpy as np
    import pandas as pd
