...
impressions, 399k queries), split 70/15/15 by query; graded labels are 5
...
selection-fold noise floor of $\pm 0.008$ to $0.015$. Enlarging the fitness fold to 6k queries halves the noise and triples mean
transferred gain, the only intervention that moved it.
...
in every seed, with unchanged transfer. A diagnostic feedback channel (noise-gated deltas, per-segment breakdowns,
...
rank features plus count encodings did ($+0.013$ NDCG@10). Building that
candidate surfaced a three-generation leakage catalog worth reusing: naive
train-fold target encoding costs $-0.10$ NDCG (the model memorizes its own
labels); the textbook leave-one-out fix is subtly worse at $-0.13$, because
subtracting the row's own label plants a per-row offset the trees read as a
label detector; query-folded out-of-fold encoding is clean. Each generation
...
property and destination identifiers, a lean feature set whose comments
cite the measured bloat evidence, and a diverse three-member ensemble under
per-query z-score weighting. One provenance
