...
PIPELINE = {
    # Three diverse members: LambdaMART (xgb rank:ndcg) as primary ranker,
    # lgbm lambdarank for implementation/family diversity, and a sklearn
    # extremely-randomized-trees regressor for a genuinely different model
    # family (not just another GBM variant), combined via per-query
    # z-score weighted sum (the challenge-winning combiner). The forest
    # member gets a lighter weight since it is weaker alone but adds
    # error diversity per the documented priors.
    "models": [
